"""Service for managing store data"""
...
class StoreService:
    ...
    async def find_or_create_store(
        ...
        try:
            ...
            query = select(Store).where(
                Store.name.ilike(f"%{normalized_name}%")
            )
            ...
            await db.rollback()
            logger.error("Store creation failed due to integrity constraint", error=str(e))
            ...
            await db.rollback()
            logger.error("Failed to create or find store", error=str(e), store_name=store_name)
            raise
    ...
    def _normalize_store_name(store_name: str) -> str:
        ...
        suffixes_to_remove = [
            'sdn bhd', 'sdn.bhd', 'ltd', 'limited', 'inc', 'incorporated',
            'corp', 'corporation', 'company', 'co.', 'pte ltd'
        ]
